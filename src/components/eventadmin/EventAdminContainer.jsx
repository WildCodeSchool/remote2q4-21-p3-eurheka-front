            ...
            <ul className="EventAdminContainerList">
                {events&&events.map((event)=>{
                    return(
                        ...
                        <li key={event.id_event}><span  className={event.isPassed?'PassedEvent':''}> {event.name} - {event.category_name} - {event.date_event}</span> <input type="button" value="Supprimer" onClick={()=>HandleDelete(event.id_event)}/></li>
                    )
                })}
            </ul>
        </div>
